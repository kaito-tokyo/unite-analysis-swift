...
作成・更新後はネイティブGoogle Docをコネクタで読み直し、構造検証を必ず行う。文書ID、意図したタイトル、親パス、Google Docs形式、見出しと本文節、冒頭と末尾の代表的な内容、期待する表数と各表の行数、本文内ハイライト画像、コンタクトシートのキャプション、期待するインライン画像数を確認する。未解決のMarkdown画像記法、プレースホルダー、Markdown表記法が残っていないことも確認する。アップロード開始だけで成功と報告しない。

ネイティブ構造検証がすべて成功した時点でGoogle Docs公開を完了としてよく、`validatedAt`へ検証時刻を記録する。PDFの書き出しや描画に成功しなくても、ネイティブ構造検証を省略または失敗扱いにしない。構造検証に失敗した場合は公開失敗として扱い、`validatedAt`を更新しない。

PDF書き出しとページ単位の目視確認は任意のbest-effort検証である。Asia/Tokyoの各日で最初に大きく更新した公開レポート、公開処理を変更した場合、または異常があった場合は試みてよい。成功時は代表ページと境界、見出し、表、画像、内容を確認し、`dailyPdfValidatedAt`へ成功時刻を記録する。書き出し、materialization、ページ描画、目視確認のいずれかに失敗または未実行の場合は警告として報告し、`dailyPdfValidatedAt`を設定または更新しない。PDF失敗だけを理由にGoogle Docs公開を失敗へ戻さない。
