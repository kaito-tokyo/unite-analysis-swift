...
- MCPが返した絶対パスをチャットクライアントが対応するローカルメディア添付として提示し、再生できたことを確認する。添付できない、再生できない、または抽出に失敗した場合は、その候補の不足理由を明示し、ソース動画画像だけをfallbackとして提示する。
...
14. 動画を主成果物、ソース動画画像を補助証拠として初回対話へ提示し、目視事実、時系列推論、分析仮説、ユーザー説明を区別して対話へ入る。
