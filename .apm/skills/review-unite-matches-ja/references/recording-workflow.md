...
1. `audio-peaks`を試合全体へ実行し、完全なraw JSONを`_PokemonUniteAnalysis/matches/<match-id>/candidates/audio-peaks.json`へ`--output`で保存する。`peaks`のscoreの95パーセンタイル以上を時刻順に並べ、最初の点を起点とする5秒未満の固定幅クラスタへまとめる。次の点が現在のクラスタの最初の点から5秒以上離れたら、新しいクラスタを始める。隣接差による推移的統合で5秒を超えるクラスタを作らない。クラスタの代表点はscoreが最大の点、同点なら早い点とし、最小時刻、最大時刻、全構成ピークも保持する。全`intervals`をそのまま候補にしない。
